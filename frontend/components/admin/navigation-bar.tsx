...
function NavigationBar({ links }: NavigationBarProps) {
  ...
  return (
    <nav className="flex w-full h-nav-bar-height pl-15 pr-30 justify-between items-center fixed top-0 z-50 bg-silver">
      <h1 className="font-bold text-xl text-black">그루터기 일일호프</h1>
      <div className="flex items-center gap-x-20">
          ...
          {links.map((link) => (
            <Link
              ...
              className={clsx("flex justify-center items-center text-black", {
                "font-bold underline decoration-2 underline-offset-4": pathname === link.href,
              })}
            >
              <p>{link.name}</p>
            </Link>
        ...
        <Link
          href="/admin/settings"
          className={clsx("p-2 rounded-full hover:bg-black/5 transition-colors", {
